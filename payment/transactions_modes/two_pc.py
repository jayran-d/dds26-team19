    ...
    while True:
        ...
        try:
            ...
            if not raw_user:
                ...
                updated_entry = _build_applied_entry(entry, "failure", PAYMENT_PREPARE_FAILED, {"reason": reason})
                pipe.multi()
                pipe.set(ledger_key, json.dumps(updated_entry), ex=payment_ledger.LEDGER_TTL_SECONDS)
                pipe.execute()
            ...
            available = user_entry.credit - _decode_int(raw_reserved_total)
            if available < amount:
            ...
            reply = build_payment_prepared(tx_id, order_id)
            updated_entry = _build_applied_entry(entry, "success", PAYMENT_PREPARED, {})
            reservation_value = json.dumps({"user_id": user_id, "amount": amount})

            pipe.multi()
            pipe.set(reservation_key, reservation_value, ex=payment_ledger.LEDGER_TTL_SECONDS)
            pipe.incrby(reserved_total_key, amount)
            pipe.set(ledger_key, json.dumps(updated_entry), ex=payment_ledger.LEDGER_TTL_SECONDS)
            pipe.execute()
            return reply, "success"
...
def _commit_payment_atomically(db: redis_module.Redis, tx_id: str, order_id: str) -> dict:
    ...
    while True:
        ...
        try:
            ...
            if not raw_commit:
                pipe.unwatch()
                raise RuntimeError(f"Missing COMMIT_PAYMENT ledger entry for tx={tx_id}")
            ...
            if raw_abort:
                ...
                if abort_entry.get("local_state") in (LedgerState.APPLIED, LedgerState.REPLIED):
                    pipe.unwatch()
                    raise RuntimeError(f"ABORT_PAYMENT already applied for tx={tx_id}")
            ...
            if not prepare_entry or prepare_entry.get("result") != "success":
                pipe.unwatch()
                raise RuntimeError(f"Missing successful PREPARE_PAYMENT for tx={tx_id}")
            ...
            if raw_reservation is None:
                pipe.unwatch()
                raise RuntimeError(f"Missing reservation during COMMIT_PAYMENT tx={tx_id}")

            raw_user = pipe.get(user_id)
            if not raw_user:
                pipe.unwatch()
                raise RuntimeError(f"Missing user row {user_id} during COMMIT_PAYMENT tx={tx_id}")

            user_entry = msgpack.decode(raw_user, type=UserValue)
            reserved_total = _decode_int(pipe.get(reserved_total_key))
            if reserved_total < amount:
                pipe.unwatch()
                raise RuntimeError(f"Reserved total for user {user_id} below tx reservation during COMMIT_PAYMENT tx={tx_id}")
            if user_entry.credit < amount:
                pipe.unwatch()
                raise RuntimeError(f"User {user_id} credit below reserved amount during COMMIT_PAYMENT tx={tx_id}")

            updated_user = UserValue(credit=user_entry.credit - amount)
            updated_entry = _build_applied_entry(commit_entry, "success", PAYMENT_COMMITTED, {})
            ...
            pipe.set(commit_key, json.dumps(updated_entry), ex=payment_ledger.LEDGER_TTL_SECONDS)
            pipe.execute()
            return reply
    ...
    while True:
        ...
        try:
            ...
            if raw_commit:
                ...
                if commit_entry.get("local_state") in (LedgerState.APPLIED, LedgerState.REPLIED):
                    pipe.unwatch()
                    raise RuntimeError(f"COMMIT_PAYMENT already applied for tx={tx_id}")
            ...
            updated_entry = _build_applied_entry(abort_entry, "success", PAYMENT_ABORTED, {})
            reply = build_payment_aborted(tx_id, order_id)

            if not prepare_succeeded or not user_id:
                pipe.multi()
                pipe.set(abort_key, json.dumps(updated_entry), ex=payment_ledger.LEDGER_TTL_SECONDS)
                pipe.execute()
                return reply
            ...
            pipe.multi()
            if raw_reservation is not None and amount > 0:
                if reserved_total < amount:
                    pipe.reset()
                    raise RuntimeError(f"Reserved total for user {user_id} below tx reservation during ABORT_PAYMENT tx={tx_id}")
                pipe.decrby(reserved_total_key, amount)
                pipe.delete(reservation_key)
            pipe.set(abort_key, json.dumps(updated_entry), ex=payment_ledger.LEDGER_TTL_SECONDS)
            pipe.execute()
...
def _handle_prepare_payment(msg, db, producer, logger):
    ...
    if abort_entry and abort_entry.get("local_state") in (LedgerState.APPLIED, LedgerState.REPLIED):
        producer.publish(PAYMENT_EVENTS_TOPIC, build_message(tx_id, order_id, abort_entry["response_event_type"], abort_entry.get("response_payload", {})))
        return
    ...
    if entry and entry.get("local_state") in (LedgerState.APPLIED, LedgerState.REPLIED):
        producer.publish(PAYMENT_EVENTS_TOPIC, build_message(tx_id, order_id, entry["response_event_type"], entry.get("response_payload", {})))
...
def _handle_commit_payment(msg, db, producer, logger):
    ...
    if not entry:
        ...
        if not created:
            entry = payment_ledger.get_entry(db, tx_id, COMMIT_PAYMENT)
            if not entry:
                logger.error(f"[Payment2PC] order={order_id} failed to create/read COMMIT ledger")
                return
